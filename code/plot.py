...
@pltdecorator
def plot_cumulative_regret(cumulative_regrets, method : str = ""):
        """
        ...
        if method != "":
            plt.title('Cumulative regret with ' + method)
        else:
            print('truc')
...
def plot_regret_eta(etas, regrets, method : str = ""):
        ...
        plt.plot(etas, regrets, label="Average regret R_T")
...
def plot_average_p_distance(ps : np.ndarray, p_star : np.ndarray, method : str = ""):
        ...
        ps is the history of weights, the average is computed inside this function
        """
        ps = np.array(ps)
        # works as expected
